...
all_res_df <- all_res_df %>% arrange(desc(estimates))
...
SHow only the four UNC13B isoforms with significant DTU in atleast one region
...
gencode_plot <- function(genename, isoforms){
  ...
  tx <- isoforms
  gff_loc$transcript_id <- factor(gff_loc$transcript_id, levels = rev(tx[order(tx)]) )
...
## Overlaps between FTLD-TDP DEGs and TDP-43 KO DEGs
...
## Overlaps between FTLD-TDP DTU genes and TDP-43 KO cryptic exon genes
